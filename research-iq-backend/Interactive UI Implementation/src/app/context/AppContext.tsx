...
import { loginApi, signupResearcherApi, signupFunderApi, mapApiUser, getCurrentUser } from '../api/auth';
import { setToken, clearToken, getToken } from '../api/client';
import { getResearchFeed, submitResearch as submitResearchApi } from '../api/research';
...
export interface ChatMessage {
  ...
  receiverId: string;
  content: string;
  /** ISO timestamp — use createdAt for display (both point to same value) */
  timestamp: string;
  createdAt: string;
  read: boolean;
...
interface AppContextType {
  user: User | null;
  /** Tries real backend first; falls back to demo accounts if backend unavailable. */
  ...
  /** True while an async login/signup call is in flight */
  authLoading: boolean;
  researchers: User[];
  research: Research[];
...
const mockChatMessages: ChatMessage[] = [
  ...
  },
  {
    ...
    createdAt: '2026-05-07T09:22:00Z',
    read: true,
  },
  {
    id: 'cm3',
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  // ── Listen for 401 force-logout from axios interceptor ───────────────────
  useEffect(() => {
    const handler = () => setUser(null);
    window.addEventListener('researchiq:logout', handler);
    return () => window.removeEventListener('researchiq:logout', handler);
  }, []);

  // ── Sync live data from backend after user logs in ────────────────────────
  useEffect(() => {
    if (!user || !getToken()) return;
    // Replace mock feed with real publications from backend
    getResearchFeed()
      .then(feed => {
        if (feed.length > 0) setResearch(feed);
      })
      .catch(() => {}); // silence — keep mock data on network failure

    // Replace mock notifications with real ones
    getNotifications()
      .then(notes => {
  ...
  const login = async (email: string, password: string): Promise<User | null> => {
    ...
    try {
      ...
      // 2. Backend unavailable — fall back to demo mock accounts
      ...
      if (password !== demoPw) return null;
      setUser(found);
      return found;
    } finally {
  ...
  const signupResearcher = async (data: Record<string, unknown>): Promise<boolean> => {
    setAuthLoading(true);
    try {
      // Try real backend signup
      await signupResearcherApi({
        ...
        publications: String(data.publications ?? '')
          .split('\n')
          .map(l => l.trim())
          .filter(Boolean),
      });
      ...
      // Fallback — add to local pending list for demo
  ...
  const signupFunder = async (data: Record<string, unknown>): Promise<boolean> => {
    ...
    try {
      await signupFunderApi({
        ...
        areasOfInterest: String(data.areasOfInterest ?? '')
          .split(',')
          .map(s => s.trim())
          .filter(Boolean),
      ...
      const newPending: PendingFunder = {
        ...
        contactPhone: String(data.contactPhone ?? ''),
        areasOfInterest: interestsRaw.split(',').map(s => s.trim()).filter(Boolean),
        investmentRange: String(data.investmentRange ?? ''),
        status: 'pending',
        submittedDate: new Date().toISOString().split('T')[0],
      };
  ...
  const approveResearcher = (id: string) => {
    ...
    adminApi.approveUser(id).catch(() => {});
  };

  const rejectResearcher = (id: string) => {
    setPendingResearchers(prev => prev.filter(p => p.id !== id));
    adminApi.rejectUser(id).catch(() => {});
  };
  ...
  const submitPublicationForReview = (data: Partial<Research> & { attachmentLabel?: string }) => {
    ...
    // Also submit to real backend (fire-and-forget)
    submitResearchApi({
      title: entry.title,
      abstract: entry.abstract,
      authors: entry.authors,
      field: entry.field,
      keywords: entry.suggestedKeywords ?? [],
  ...
  const approvePublication = (id: string) => {
    ...
    adminApi.approvePublication(id).catch(() => {});
  };

  const rejectPublication = (id: string) => {
    setPendingPublications(prev => prev.filter(x => x.id !== id));
    adminApi.rejectPublication(id).catch(() => {});
  };
  ...
  const sendChatMessage = (receiverId: string, content: string, attachmentName?: string) => {
    ...
    // Also send to real backend (fire-and-forget)
    apiSendMessage(receiverId, content).catch(() => {});
  };

...
import { Brain, MapPin, Users, TrendingUp, Search, Download, BarChart3, Sparkles, Network } from 'lucide-react';
...
export function ExpertiseMap() {
  ...
  const [activeTab, setActiveTab] = useState<'heatmap' | 'experts' | 'similarity' | 'trends'>('heatmap');
  const [selectedKeyword, setSelectedKeyword] = useState<string | null>(null);

  // ── AI data from backend microservices ──────────────────────────────────
  const [aiHeatmap, setAiHeatmap] = useState<aiApi.ExpertiseHeatmapEntry[]>([]);
  const [aiNetworkSummary, setAiNetworkSummary] = useState<aiApi.NetworkSummary | null>(null);
  ...
  useEffect(() => {
    ...
    setAiLoading(true);
    Promise.allSettled([
      aiApi.getExpertiseHeatmap().then(setAiHeatmap),
      aiApi.getNetworkSummary().then(setAiNetworkSummary),
  ...
  return (
    ...
    <div style={{ padding: '2rem' }}>
        <div className="mb-6 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-gradient-to-br from-blue-800 to-green-600 rounded-lg flex items-center justify-center">
                <Brain className="w-6 h-6 text-white" />
              </div>
              <h1 className="text-3xl font-bold">Institutional Expertise Map</h1>
            </div>
        ...
        {/* AI Network Summary (from AI service 8006) */}
        {aiNetworkSummary ? (
          <div className="grid grid-cols-4 gap-4 mb-6">
            {[
              { label: 'Nodes (Researchers)', value: aiNetworkSummary.totalNodes, icon: Users, color: 'text-blue-800' },
              { label: 'Collaboration Edges', value: aiNetworkSummary.totalEdges, icon: Network, color: 'text-green-700' },
              { label: 'Avg. Connections', value: aiNetworkSummary.avgDegree.toFixed(1), icon: TrendingUp, color: 'text-purple-700' },
              { label: 'Communities Detected', value: aiNetworkSummary.communities?.length ?? 0, icon: Sparkles, color: 'text-orange-600' },
            ].map(stat => {
              const Icon = stat.icon;
              return (
                <Card key={stat.label} className="p-4 border-l-4 border-blue-800">
          ...
          <div className="mb-6 p-4 bg-blue-50 rounded-xl flex items-center gap-3 text-sm text-blue-700">
            <Sparkles className="w-5 h-5 animate-pulse" />
            Loading AI network analysis from backend…
          </div>
        ) : null}
                ...
                {expertiseAreas.map(area => {
                  ...
                  return (
                    <button
                      ...
                      className={`aspect-square rounded-xl p-3 flex flex-col items-center justify-center text-center transition-all hover:scale-105 border-2 ${selectedKeyword === area.keyword ? 'border-blue-800 ring-2 ring-blue-300' : 'border-transparent'}`}
                  ...
                  {selectedExperts.map(e => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Badge className="bg-blue-100 text-blue-700 text-xs">{e.count} publication{e.count > 1 ? 's' : ''}</Badge>
                      </button>
                    );
                  })}
                </div>
              ...
              <Card className="p-6">
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={expertiseAreas.slice(0, 8)} layout="vertical" margin={{ left: 8 }}>
                    ...
                    <Bar dataKey="expertCount" fill="#1E40AF" radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </Card>
                    ...
                    {expertSearchResults.map(({ researcher: r, keywords, matchedKw }) => (
                      <Card key={r.id} className="p-5 hover:shadow-lg transition-all border border-gray-100">
                        <div className="flex items-start gap-3 mb-3">
                          <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-green-500 rounded-full flex items-center justify-center text-white font-bold text-lg shrink-0">
                            {r.name.charAt(0)}
                          </div>
                        ...
                        <Button size="sm" className="w-full bg-gradient-to-r from-blue-800 to-green-600 hover:from-blue-700 hover:to-green-700" onClick={() => navigate(`/researcher/profile/${r.id}`)}>
                          View Profile
                        </Button>
                      </Card>
                    ))}
                  ...
                  {similarityPairs.map((pair, idx) => (
                    <div key={idx} className="p-5 border border-gray-200 rounded-xl hover:border-blue-300 hover:shadow-md transition-all">
                      ...
                      <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden mb-3">
                        <div className="h-full bg-gradient-to-r from-blue-800 to-green-600 rounded-full" style={{ width: `${pair.score}%` }} />
                      </div>
                      <div className="flex flex-wrap gap-1">
                        <span className="text-xs text-gray-500 mr-1">Shared keywords:</span>
                        {pair.shared.map(k => (
                          <Badge key={k} className="bg-blue-50 text-blue-700 text-xs">{k}</Badge>
                        ))}
                      </div>
                    </div>
                  ))}

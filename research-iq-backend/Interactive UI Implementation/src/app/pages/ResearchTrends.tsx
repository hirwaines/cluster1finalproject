...
import { TrendingUp, TrendingDown, Download, BarChart3, Sparkles } from 'lucide-react';
import {
...
export function ResearchTrends() {
  ...
  // AI trend data from backend (AI service 8003)
  const [aiTrends, setAiTrends] = useState<aiApi.TrendAnalysis | null>(null);
  const [aiEmergingAreas, setAiEmergingAreas] = useState<{ area: string; growthRate: number; confidence: number }[]>([]);
  ...
  useEffect(() => {
    ...
    setAiLoading(true);
    Promise.allSettled([
      aiApi.getFullTrendAnalysis().then(setAiTrends),
      aiApi.getEmergingAreas(3).then(setAiEmergingAreas),
  ...
  return (
    ...
    <div style={{ padding: '2rem' }}>
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold mb-1">Research Trend Analysis</h1>
        ...
        {/* AI Emerging Areas panel — from AI service 8003 */}
        {(aiEmergingAreas.length > 0 || aiLoading) && (
          <div className="mb-6 p-5 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="font-semibold text-blue-900 text-sm">AI-Detected Emerging Research Areas</span>
              <Badge className="bg-blue-800 text-white text-xs ml-auto">Powered by AI Service</Badge>
            </div>
            {aiLoading ? (
              <p className="text-sm text-blue-600 animate-pulse">Analysing trends across the research corpus…</p>
            ) : (
              <div className="flex flex-wrap gap-3">
                {aiEmergingAreas.slice(0, 6).map(area => (
                  <div key={area.area} className="flex items-center gap-2 bg-white rounded-lg px-3 py-2 shadow-sm border border-blue-100">
                    <TrendingUp className="w-3.5 h-3.5 text-green-600" />
                    <span className="text-sm font-medium">{area.area}</span>
                    <span className="text-xs text-green-700 font-bold">+{(area.growthRate * 100).toFixed(0)}%</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <Card className="p-6">
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <AreaChart data={pubsByMonth}>
                    ...
                    <Tooltip />
                    <Area type="monotone" dataKey="publications" stroke="#1E40AF" fill="url(#pubGrad)" strokeWidth={2} name="Publications" />
                  </AreaChart>
                </ResponsiveContainer>
              </Card>
                ...
                {hotTopics.map((topic, idx) => {
                  ...
                  return (
                    <div key={topic.keyword} className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl hover:border-blue-300 hover:shadow-md transition-all">
                      <div className="w-8 h-8 bg-[#1E40AF] rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0">
                        {idx + 1}
                      </div>
              ...
              {emergingTopics.length > 0 ? (
                <div className="flex flex-wrap gap-3">
                  {emergingTopics.map(topic => (
                    <div key={topic} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-full">
                ...
                {research.sort((a, b) => b.citations - a.citations).map(pub => (
                  <div key={pub.id} className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl hover:border-blue-300 transition-all">
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold truncate text-blue-700">{pub.title}</div>
                      <div className="text-sm text-gray-500">{pub.authors.join(', ')} Â· {pub.publicationDate}</div>
          ...
          <div className="space-y-6">
            <Card className="p-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                <Badge className="bg-blue-100 text-blue-700">8% monthly growth model</Badge>
              </div>
              <p className="text-sm text-gray-600 mb-6">
                Forecast uses linear extrapolation from the last 3 months of publication data. Shaded area shows confidence interval (Â±50% growth rate variance).
              </p>
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={forecastData}>
                  ...
                  <YAxis allowDecimals={false} />
                  <Tooltip />
                  <ReferenceLine x={pubsByMonth[pubsByMonth.length - 1]?.month} stroke="#9CA3AF" strokeDasharray="4 4" label={{ value: 'Forecast â†’', position: 'top', fontSize: 11, fill: '#6B7280' }} />
                  <Area type="monotone" dataKey="publications" stroke="#1E40AF" fill="url(#forecastGrad)" strokeWidth={2} name="Actual" connectNulls={false} />

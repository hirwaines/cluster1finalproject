...
export function SignUpResearcherPage() {
  ...
  const getPasswordStrength = (pw: string) => {
    ...
    if (score === 4) return { score, label: 'Good', color: 'bg-[#1E40AF]', textColor: 'text-[#1E40AF]' };
    return { score, label: 'Strong', color: 'bg-[#047857]', textColor: 'text-[#047857]' };
  };
  ...
  const sendVerificationCode = async () => {
    ...
    try {
      ...
      // Fallback: local demo code
      const code = Math.floor(100000 + Math.random() * 900000).toString();
      setVerificationCode(code);
      toast.success(`Verification code sent to ${formData.email} (demo: ${code})`);
    }
    setCodeSent(true);
  };
  ...
  const verifyEmailCode = async () => {
    try {
      await verifyEmailOtp(formData.email, enteredCode);
      setEmailVerified(true);
      toast.success('Email verified successfully!');
    } catch {
      // Fallback to local demo code check
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="mb-10 flex items-center gap-4">
          ...
          <div className="flex-1 h-1 bg-gray-200 rounded">
            <div className={`h-full bg-[#1E40AF] rounded transition-all ${step > 1 ? 'w-full' : 'w-0'}`} />
          </div>
          <StepIndicator number={2} title="Credentials" active={step === 2} completed={step > 2} />
          <div className="flex-1 h-1 bg-gray-200 rounded">
            <div className={`h-full bg-[#1E40AF] rounded transition-all ${step > 2 ? 'w-full' : 'w-0'}`} />
          </div>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    {formData.confirmPassword && (
                      <p className={`text-xs mt-1 ${formData.password === formData.confirmPassword ? 'text-[#047857]' : 'text-red-600'}`}>
                ...
                <div className="p-4 border rounded-lg bg-gray-50 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {emailVerified && (
                      <span className="text-[#047857] text-sm font-medium flex items-center gap-1">
                ...
                <Button type="button" style={{ backgroundColor: '#1E40AF' }} className="w-full hover:opacity-90" onClick={() => setStep(2)}>
                  Continue
                </Button>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-6">
                ...
                <div className="bg-[#EFF6FF] border border-[#BFDBFE] rounded-lg p-4 flex gap-3 text-sm text-[#1E3A8A]">
                ...
                <div className="flex gap-3">
                  ...
                  <Button type="button" style={{ backgroundColor: '#1E40AF' }} className="flex-1 hover:opacity-90" onClick={() => setStep(3)}>Continue</Button>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-6">
                ...
                <div className="flex gap-3">
                  ...
                  <Button type="submit" style={{ backgroundColor: '#1E40AF' }} className="flex-1 hover:opacity-90">Submit application</Button>
                </div>
              </div>
            )}
          </form>
    ...
    <div className="flex flex-col items-center flex-1">
      <div
        className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm mb-1 ${
          completed || active ? 'bg-[#1E40AF] text-white' : 'bg-gray-200 text-gray-500'
        }`}
      >
        {completed ? <CheckCircle2 className="w-5 h-5" /> : number}
      </div>
      <span className={`text-xs font-medium ${active || completed ? 'text-[#1E40AF]' : 'text-gray-400'}`}>{title}</span>
    </div>
  );
}
